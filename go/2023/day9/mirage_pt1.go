...
func predict(hist [][]int) int {
    ...
    for _, cur := range hist {
        ...
        for i > 0 {
            target := datasets[i][len(datasets[i]) - 1]
            prediction := datasets[i - 1][len(datasets[i - 1]) - 1] + target
            datasets[i - 1] = append(datasets[i - 1], prediction)

            i -= 1
        }
        sum += datasets[0][len(datasets[0]) - 1]
    }
...
func main() {
    ...
    sum := predict(hist)
    if sum != 114 {
        panic(fmt.Sprintf("Test case failed! Expected: 114, computed: %v", sum))
    }
